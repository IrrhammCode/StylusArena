...
function HistoryList() {
   ...
   useEffect(() => {
      ...
      // Build set of deployed training IDs
      const deployedSet = new Set<string>(deployed.map((a: any) => a.trainingId).filter(Boolean))
      setDeployedIds(deployedSet)

      setModels(saved.reverse())
   }, [])

   if (models.length === 0) return <div className="text-gray-500 italic px-2">No past training data found.</div>

   return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {models.map((model, i) => {
            const isDeployed = deployedIds.has(model.id)
            const href = isDeployed
               ? `/agents/analytics?trainingId=${model.id}`
               : `/agents/deploy?trainingId=${model.id}`

            return (
               <Link href={href} key={`${model.id}-${i}`}>
                  <motion.div
                     whileHover={{ y: -5 }}
                     className={`bg-[#0F1422] p-6 rounded-2xl border ${isDeployed ? 'border-green-500/30' : 'border-[#1A1F3A]'} hover:border-arbitrum-cyan/50 group cursor-pointer`}
                  >
                     <div className="flex justify-between items-start mb-4">
                        <div className={`p-3 ${isDeployed ? 'bg-green-500/10' : 'bg-blue-500/10'} rounded-xl group-hover:bg-blue-500/20 transition-colors`}>
                           <ZapIcon className={`w-6 h-6 ${isDeployed ? 'text-green-400' : 'text-blue-400'}`} />
                        </div>
                        <div className="flex flex-col items-end gap-1">
                           <span className="text-xs font-mono text-gray-500">{new Date(model.timestamp).toLocaleDateString()}</span>
                           {isDeployed && (
                              <span className="text-[10px] px-2 py-0.5 bg-green-500/20 text-green-400 rounded border border-green-500/30">DEPLOYED</span>
                           )}
                        </div>
                     </div>
                     <h4 className="text-lg font-bold text-white mb-1 capitalize">{model.gameType || 'Agent'} Model</h4>
                     <div className="flex gap-4 text-sm text-gray-400 mb-4">
                        <div>Acc: <span className="text-white font-mono">{(model.accuracy * 100).toFixed(1)}%</span></div>
                        <div>Loss: <span className="text-white font-mono">{model.loss.toFixed(4)}</span></div>
                     </div>
                     <div className={`${isDeployed ? 'text-green-400' : 'text-arbitrum-cyan'} text-sm font-bold flex items-center gap-2`}>
                        {isDeployed ? (
                           <>View Analytics <CheckCircleIcon className="w-4 h-4" /></>
                        ) : (
                           <>Deploy Now <RocketIcon className="w-4 h-4" /></>
                        )}
                     </div>
                  </motion.div>
               </Link>
...
function TrainingContent() {
   ...
   const gameTypeParam = searchParams.get('game') || 'racing'
   ...
   useEffect(() => {
      if (trainingId) {
         ...
         setTrainingStatus({
            trainingId,
            gameType: gameTypeParam, // Dynamic game type
            status: 'in_progress',
            progress: 0,
            epoch: 0,
            totalEpochs,
         ...
         <main className="max-w-7xl mx-auto px-6 -mt-24 relative z-20 pb-20">
            {isLoading ? (
               ...
               <div className="flex flex-col gap-12">
                  <div className="text-center py-20 bg-[#0F1422]/50 rounded-3xl border border-[#1A1F3A] border-dashed">
                     <div className="w-24 h-24 mx-auto mb-6 rounded-full bg-[#1A1F3A] flex items-center justify-center shadow-lg">
                        <span className="text-4xl">🤖</span>
                     </div>
                     <h3 className="text-3xl font-bold text-white mb-2">Start New Training</h3>
                  ...
                  <div className="space-y-6">
                     <h3 className="text-2xl font-bold text-white px-2">Recent Training Sessions</h3>
                     {/* We need to fetch this from localStorage inside the component */}
                     <HistoryList />
                  </div>
               </div>
            )}
